...
warnings.filterwarnings("ignore", category=UserWarning, message="Defining your `__torch_function__`.*")
...
def layer_pruning(model, param_path, pruning_percentage=0.5, pruning_type='l1'):
    ...
    print(f"Pruning {param_path} | Shape: {tuple(getattr(target_module, param_name).shape)} | Elements: {getattr(target_module, param_name).numel()}")

    param = getattr(target_module, param_name)
    if not isinstance(param, torch.nn.Parameter):
        raise ValueError(f"Parameter '{param_name}' in module '{module_path}' is not a torch.nn.Parameter")
    if param.numel() == 0:
        raise ValueError(f"Parameter '{param_name}' in module '{module_path}' has no elements to prune")

    # Calculate the maximum allowed pruning percentage to keep at least 20 parameters
    if pruning_percentage > 0.0:
        max_pruning_percentage = 1.0 - (64.0 / param.numel())  # Ensure at least 20 params remain
        adjusted_pruning_percentage = min(pruning_percentage, max_pruning_percentage)
        
        if adjusted_pruning_percentage <= 0.0:
            print(f"Skipping pruning for {param_path} as pruning would leave fewer than 20 parameters")
            return model_copy
        elif adjusted_pruning_percentage < pruning_percentage:
            print(f"Adjusting pruning percentage for {param_path} from {pruning_percentage} to {adjusted_pruning_percentage} to keep at least 20 parameters")
            pruning_percentage = adjusted_pruning_percentage
...
def sensitivity_analysis(model, test_loader, sparsity_info, pruning_type='l1', step = 20):
    ...
    for param in sparsity_info:
        ...
        if layer_type in ['QuantLinear', 'QuantConv2d']:
            for percentage in range(0, 1000, step):
                percentage /= 1000.0
                # Apply pruning
...
def retrain_model(model, train_loader, val_loader, epochs=20, lr=0.01, 
                 weight_clipping=None, is_binary=None):
    ...
    if is_binary is None:
        try:
            dummy_input = torch.randn(2, *next(iter(train_loader))[0].shape[1:]).to(device)
            model.eval()
            with torch.no_grad():
                output_size = model(dummy_input).shape[-1]
            is_binary = output_size == 1
            model.train()
        except Exception as e:
            print(f"自动检测失败: {e}, 默认使用二分类")
            is_binary = True
...
def auto_prune_model_sensitivity(ori_model, model_name, w, a, dataset_name='MNIST', pruning_type='l1'):
    ...
    accuracy_drop_tolerance = 50  # 40% layer accuracy drop tolerance
...
if __name__ == "__main__":
    ...
    # a = 2
    # model_name = "unsw_fc"
    # model_weight = "./model/best_unsw_fc_w2_a2_1000.pth"
    ...
    # ori_model.load_state_dict(torch.load(model_weight))
    # ori_model.to(device)
    # auto_prune_model(ori_model, model_name, w, a, dataset_name='UNSW', pruning_type='l1')
    w = 4
    a = 4
    model_name = "2c3f_relu"
    model_weight = "./model/best_2c3f_relu_w4_a4_500.pth"
